    ...
    try:
        ...
        if is_vol or is_kd:
            ...
            return {
                ...
                "波段賣出": round(float(curr['Close'] + (df['ATR'].iloc[-1] * 2.1)), 2),
...
with st.sidebar:
    st.header("⚙️ 參數設定")
    vol_target = st.slider("1. 量能倍數", 0.1, 3.0, 0.5, 0.1)
    kd_limit = st.slider("2. KD 門檻", 10, 95, 75, 5)
    scan_limit = st.number_input("3. 掃描檔數", 10, 2800, 500)
