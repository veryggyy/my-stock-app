...
def get_full_taiwan_list():
    ...
    if os.path.exists(file_path):
        try:
            # 使用 utf-8-sig 讀取，相容 Excel 存出的 CSV
                ...
                for line in lines:
                    ...
                    if len(parts) >= 2:
                        ...
                        if "代號" not in code and len(code) >= 4:
                            ...
                            stocks[code] = name
            if len(stocks) > 0:
                return stocks, f"✅ 已從本地載入清單 (共 {len(stocks)} 檔)"
        except Exception as e:
            return {"2330.TW": "台積電"}, f"❌ 讀取本地檔案失敗: {str(e)}"
    
    return {"2330.TW": "台積電", "2317.TW": "鴻海"}, "⚠️ 找不到本地 CSV 檔案，使用保底清單"
...
with st.sidebar:
    st.header("⚙️ 篩選門檻")
    ret_target = st.slider("今日漲幅門檻 (%)", 0.5, 7.0, 2.0, 0.5)
    ...
    if results:
        ...
        for item in results:
            with st.container(border=True):
                ...
                st.markdown(f'<div class="price-box">🟢 月線: {item["月線"]} | 趨勢: ↗ 向上</div>', unsafe_allow_html=True)
    else:
        st.error("❌ 目前條件下無符合標的。")
